...
public class EgovConfigWebDispatcherServlet implements WebMvcConfigurer {
	...
	@Autowired
	EgovWebLogService egovWebLogService;

	//private final String[] CORS_ORIGIN_SERVER_URLS = {"http://172.30.1.89:3000", "http://localhost:3000"};
...
//				"/cop/bbs/*Master*.do",
		...
		EgovWebLogInterceptor egovbWebLogInterceptor = new EgovWebLogInterceptor();
		egovbWebLogInterceptor.setWebLogService(egovWebLogService);
		registry.addInterceptor(egovbWebLogInterceptor).addPathPatterns("/**/*.do");
	...
	//	registry.addMapping("*.do").allowedHeaders("*");
	//}
